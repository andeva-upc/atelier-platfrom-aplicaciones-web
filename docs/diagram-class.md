...
*   **Enumeración: `ElectronicBillingStatus`**
    *   **Valores:** `PENDING`, `PROCESSING`, `ACCEPTED`, `REJECTED`
...
*   **Clase: `Voucher`** `<<AggregateRoot>>`
    *   **Atributos:**
        ...
        *   `- tax: TaxRate`
        *   `- total: Money`
        *   `- billingStatus: ElectronicBillingStatus`
        *   `- items: List<VoucherItem>`
    *   **Métodos:** `+ createInvoice(subtotal: Money, tax: TaxRate): Voucher`, `+ addItem(item: VoucherItem): void`, `+ updateBillingStatus(status: ElectronicBillingStatus): void`
...
*   **Clase: `EventPublisherService`** `<<Component>>`
    ...
    *   **Métodos:** `+ publishInvoiceCreated(event: VoucherCreatedEvent): void COMMENT 'Publica comprobante.pendiente en RabbitMQ para ser consumido asíncronamente'`, `+ publishDtcAlertDetected(alert: VehicleDtcAlertEvent): void`
*   **Interface: `IElectronicBillingPort`**
    *   **Métodos:** `+ issueVoucher(voucher: Voucher): String COMMENT 'Retorna el XML/Respuesta de Facthub'`
*   **Clase: `FacthubClientAdapter`** `<<Component>>`
    *   **Atributos:** `- restTemplate: RestTemplate`
    *   **Métodos:** `+ issueVoucher(voucher: Voucher): String COMMENT 'Llama al POST /api/invoices/issue de facthub-service'`
*   **Clase: `FacthubWorker`** `<<Component>>`
    *   **Atributos:** `- billingPort: IElectronicBillingPort`, `- voucherRepo: IVoucherRepository`
    *   **Métodos:** `+ handleInvoiceCreatedEvent(event: VoucherCreatedEvent): void COMMENT 'Escucha RabbitMQ, llama a Facthub y actualiza el estado a ACCEPTED/REJECTED'`
...
5.  **De `Voucher` a `ElectronicBillingStatus`**
    *   **Tipo:** Composición
    *   **Dirección:** De Voucher hacia ElectronicBillingStatus
    *   **Calificación:** "has status with Facthub/SUNAT"
    *   **Multiplicidad:** 1 (Voucher) a 1 (ElectronicBillingStatus)
...
20. **De `FacthubWorker` a `IElectronicBillingPort`**
    *   **Tipo:** Dependencia (Uso)
    *   **Dirección:** De FacthubWorker hacia IElectronicBillingPort
    *   **Calificación:** "issues invoices through"
    *   **Multiplicidad:** 1 (FacthubWorker) a 1 (IElectronicBillingPort)
21. **De `FacthubWorker` a `IVoucherRepository`**
    *   **Tipo:** Dependencia (Uso)
    *   **Dirección:** De FacthubWorker hacia IVoucherRepository
    *   **Calificación:** "updates status in"
    *   **Multiplicidad:** 1 (FacthubWorker) a 1 (IVoucherRepository)
22. **De `FacthubClientAdapter` a `IElectronicBillingPort`**
    *   **Tipo:** Realización (Implementación)
    *   **Dirección:** De FacthubClientAdapter hacia IElectronicBillingPort
    *   **Calificación:** "implements"
    *   **Multiplicidad:** (Sin cardinalidad de instancia)
