...
public class OBD2DeviceRegistrationRepository : BaseRepository<OBD2DeviceRegistration>, IOBD2DeviceRegistrationRepository
{
    ...
    public async Task<OBD2DeviceRegistration?> FindByVehicleIdAndStatusAsync(Guid vehicleId, OBD2DeviceRegistrationStatus status, CancellationToken cancellationToken = default)
    {
        return await Context.Set<OBD2DeviceRegistration>()
            .FirstOrDefaultAsync(r => r.VehicleId == vehicleId && r.Status == status, cancellationToken);
    }

    public async Task<IEnumerable<OBD2DeviceRegistration>> FindByBranchIdAsync(Guid branchId, CancellationToken cancellationToken = default)
    {
        return await Context.Set<OBD2DeviceRegistration>()
