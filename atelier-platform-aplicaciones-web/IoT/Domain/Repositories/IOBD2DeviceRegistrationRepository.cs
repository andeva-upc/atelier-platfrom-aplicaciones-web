...
public interface IOBD2DeviceRegistrationRepository : IBaseRepository<OBD2DeviceRegistration>
{
    ...
    Task<OBD2DeviceRegistration?> FindByVehicleIdAndStatusAsync(Guid vehicleId, OBD2DeviceRegistrationStatus status, CancellationToken cancellationToken = default);
    Task<IEnumerable<OBD2DeviceRegistration>> FindByBranchIdAsync(Guid branchId, CancellationToken cancellationToken = default);
}
